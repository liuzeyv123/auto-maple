...
class Bot(Configurable):
    ...
    def _main(self):
        ...
        while True:
            try:
                ...
                # 每3分钟进行一次深度垃圾回收
                memory_counter += 1
                if memory_counter >= 9000:  # 50fps * 60s * 3min = 9000
                    print("[Bot] 执行深度垃圾回收...")
                    import gc
                    gc.collect()
                    memory_counter = 0
    ...
    def _solve_rune(self):
        ...
        # 尝试3次识别符文
        for i in range(3):
            # 获取当前游戏画面（直接使用原始帧，不复制）
            rune_frame = config.capture.frame if config.capture.frame is not None else None
            # 使用预测API获取符文解决方案
            ...
            if solution and len(solution) == 4:
                ...
                for _ in range(3):
                    time.sleep(0.5)
                    # 获取当前游戏画面（直接使用原始帧，不复制）
                    frame = config.capture.frame if config.capture.frame is not None else None
                    if frame is not None:
                        # 直接使用切片，不复制
                        search_region = frame[:frame.shape[0] // 8, :]
                        
                        rune_buff = utils.multi_match_color(search_region, RUNE_BUFF_TEMPLATE, threshold=0.7)
                        if len(rune_buff)>1:
    ...
    def close(self):
        ...
        # 释放其他资源
        if hasattr(self, 'command_book'):
            self.command_book = None
        if hasattr(self, 'last_position'):
            self.last_position = None
        if hasattr(self, 'rune_active'):
            self.rune_active = False
        if hasattr(self, 'rune_pos'):
            self.rune_pos = None
        if hasattr(self, 'rune_closest_pos'):
            self.rune_closest_pos = None
