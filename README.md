...
Auto Maple 采用模块化设计，只要提供游戏内动作列表或"命令书"，就可以操作游戏中的任何角色。命令书是一个 Python 文件，包含多个类，每个类对应一个游戏内能力，告诉程序应该按哪些键以及何时按这些键。导入命令书后，其类会自动编译成一个字典，Auto Maple 可以使用该字典来解释例程中的命令。命令可以访问 Auto Maple 的所有全局变量，这使它们能够根据玩家的位置和游戏状态主动改变行为
  ...
  <br>闪现职业命令书参考<a href = "https://github.com/liuzeyv123/auto-maple-resources/blob/73361f3fd76f1fcd37d5b24bc95fbeaf87afc912/command_books/fpmagic.py">F/P Magic</a>
  <br>二段跳的跳A职业参考<a href = "https://github.com/liuzeyv123/auto-maple-resources/blob/73361f3fd76f1fcd37d5b24bc95fbeaf87afc912/command_books/hayato.py">hayato</a>  
</table>
...
<ol>
  ...
  <li>
    克隆或下载仓库代码 <a href="https://github.com/liuzeyv123/auto-maple/releases"></a>。
  ...
  <li>
    （可选）首次使用时，如果启动报错 <code>Cannot create context</code>，需以<strong>管理员身份</strong>在项目根目录执行以下命令安装 Interception 内核驱动：
    <pre><code>& "Interception/command line installer/install-interception.exe" /install</code></pre>
    如果之前已经安装过 Interception 驱动，则无需重复执行此步骤。
  </li>
  <li>
    最后，通过运行创建桌面快捷方式：
